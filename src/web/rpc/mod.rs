...
use crate::{
	...
	model::ModelManager,
	web::{
		rpc::rpc_task::{create_task, delete_task, list_tasks, update_task},
		Error, Result,
	},
};
use axum::{
...
async fn rpc_handler(
	...
	// -- Create RPC Info to be set to the response.extensions
	let rpc_info = RpcInfo {
		id: rpc_req.id.clone(),
		method: rpc_req.method.clone(),
	};

	// -- Exec and store RpcInfo
...
async fn _rpc_handler(
	...
	debug!(
		"->> {:<12} - _rpc_handler - method: {rpc_method}",
		"HANDLER"
	);
	...
	let result_json: Value = match rpc_method.as_str() {
		...
		// -- Fallback
		_ => return Err(Error::RpcMethodUnknown(rpc_method.to_string())),
	};
